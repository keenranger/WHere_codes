...
class HeadingCalculator:
    ...
    def step(self, time, gyro, rot_vec, game_rot_vec):
        ...
        if not np.isnan(self.step_before[0]):
            ...
            heading = np.rad2deg(-rot_vec_orientation[0])
            processed_heading = np.rad2deg(-rot_vec_orientation[0])
        self.step_before = [time, gyro[2], processed_gyro[2]]

        self.heading_df.loc[len(self.heading_df)] = [time, np.rad2deg(heading), np.rad2deg(
            processed_heading), np.rad2deg(-rot_vec_orientation[0])]
