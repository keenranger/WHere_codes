...
class PeakValleyPlotter():
    def __init__(self, pvdetect, norm_df, file_name):
        ...
        plt.plot(self.norm_df['time'],
                 self.norm_df['value'], c='y', linewidth=0.5)
        plt.title(file_name)
    ...
    def ani(self):
        ...
        temp2 = plt.scatter(
            self.norm_df['time'].loc[0], self.norm_df['value'].loc[0], c='r')
        temp3 = plt.scatter(
            self.norm_df['time'].loc[0], self.norm_df['value'].loc[0], c='g')

        def animate(i):
            temp.set_data([self.norm_df['time'].loc[i*10],
                           self.norm_df['time'].loc[i*10]], [0, 1])
            peak_now = (
                self.pvdetect.peak_df[self.pvdetect.peak_df['time'] < self.norm_df.loc[i*10]['time']])
            valley_now = (
                self.pvdetect.valley_df[self.pvdetect.valley_df['time'] < self.norm_df.loc[i*10]['time']])
    ...
    def ani_save(self):
        ...
        myAnimation.save("./saves/" + self.file_name +
                         '.gif', writer='imagemagick', fps=30)
